...
class Rawly {
   ...
   * @param  {Boolean}     overwrite=false - Overwrite existing previews
   * @return {Promise}                       Returns Promise if files are extracted
   */
  extractPreviews(id, overwrite = false) {
    if (this.previewsExtracted) {
      Promise.resolve(`Previews for ${this.name}.${this.ext} are already extracted`);
    }
